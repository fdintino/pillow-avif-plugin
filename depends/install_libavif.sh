...
if [ "$LIBAVIF_VERSION" != "0.11.0" ]; then
    ...
    HAS_EXT_DIR=
    # libavif >= 1.0 uses OFF/LOCAL/SYSTEM for codec options; ON is ignored
    SYSTEM_CODEC=SYSTEM
else
    SYSTEM_CODEC=ON
fi
...
if $PKGCONFIG --exists dav1d; then
    LIBAVIF_CMAKE_FLAGS+=(-DAVIF_CODEC_DAV1D=$SYSTEM_CODEC)
    HAS_DECODER=1
fi

if $PKGCONFIG --exists rav1e; then
    LIBAVIF_CMAKE_FLAGS+=(-DAVIF_CODEC_RAV1E=$SYSTEM_CODEC)
    HAS_ENCODER=1
fi

if $PKGCONFIG --exists SvtAv1Enc; then
    LIBAVIF_CMAKE_FLAGS+=(-DAVIF_CODEC_SVT=$SYSTEM_CODEC)
    HAS_ENCODER=1
fi

if $PKGCONFIG --exists libgav1; then
    LIBAVIF_CMAKE_FLAGS+=(-DAVIF_CODEC_LIBGAV1=$SYSTEM_CODEC)
    HAS_DECODER=1
fi

if $PKGCONFIG --exists aom; then
    LIBAVIF_CMAKE_FLAGS+=(-DAVIF_CODEC_AOM=$SYSTEM_CODEC)
    HAS_ENCODER=1
    HAS_DECODER=1
fi
